...
pub mod csbotnet {
    ...
    use crate::{
        environment::Environment, init_values, threadpool::ThreadPool, THREAD_POOL, WEIGHT,
    };
    ...
    pub fn get_best_move(
        ...
        let mut environment = Environment::new();
        environment.init_with_data(current, hold, field, next);

        let result = environment.search();
